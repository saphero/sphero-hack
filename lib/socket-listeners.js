    ...
    socket.on('roll', (data) => {
      ...
      switch (data.direction) {
        case 'left':
          ...
          interval = setInterval(() => rollDirection(orb, true, 270, socket), 1000);
          break;
        case 'up':
          rollDirection(orb, false, 0, socket);
          interval = setInterval(() => rollDirection(orb, false, 0, socket), 1000);
          break;
        case 'right':
          rollDirection(orb, true, 90, socket);
          interval = setInterval(() => rollDirection(orb, true, 90, socket), 1000);
          break;
        case 'down':
          rollDirection(orb, true, 180, socket);
          interval = setInterval(() => rollDirection(orb, true, 180, socket), 1000);
          break;
        case 'stop':
